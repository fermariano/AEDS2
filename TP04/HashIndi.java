...
class Lista {
    ...
    boolean pesquisar(String nome) {
        Celula i = inicio;
        while (i != null) {
            if (i.getElemento().getNome().equals(nome)) {
                return true;
            }
            i = i.getProx();
        }
...
public class HashTable {
    ...
    int calcPos(Jogador usuario) {
        return usuario.getAltura() % tam;
    }

    int rehash(Jogador usuario) {
        return (usuario.getAltura() + 1) % tam;
    }

    void add(Jogador x) {
        hash[calcPos(x)].inserir(x);
    }
