...
void quick(int esq, int dir) {
  int i = esq, j = dir;
  Jogador pivo = arrJogadores[(dir+esq)/2];
  while (i <= j) {
    while (strcmp(getEstadoNascimento(arrJogadores[i]), getEstadoNascimento(pivo)) < 0 || strcmp(getEstadoNascimento(arrJogadores[i]), getEstadoNascimento(pivo)) == 0 && strcmp(getNome(arrJogadores[i]), getNome(pivo)) < 0) {
      i++;
    } 
    while (strcmp(getEstadoNascimento(arrJogadores[j]), getEstadoNascimento(pivo)) > 0 || strcmp(getEstadoNascimento(arrJogadores[j]), getEstadoNascimento(pivo)) == 0 && strcmp(getNome(arrJogadores[j]), getNome(pivo)) > 0) {
      j--;
    } 
    nComparacoes++;
    if (i <= j) {
...
void logs() {
  const char *nomeArquivo = "matrícula_quicksort.txt";
  char content[100];
