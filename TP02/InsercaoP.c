...
void insertionParcial() {
  int k = 10;
    for (int i = 1; i < nArrJogadores; i++) {
        Jogador tmp = arrJogadores[i];
        int j = (i < k) ? i-1 : k -1;
        while ((j >= 0) && getAnoNascimento(arrJogadores[j]) > getAnoNascimento(tmp) || getAnoNascimento(arrJogadores[j]) == getAnoNascimento(tmp) && strcmp(getNome(arrJogadores[j]), getNome(tmp)) > 0) {
          arrJogadores[j+1] = arrJogadores[j];
          j--;
        }
        arrJogadores[j+1] = tmp;
  }
}
...
void logs() {
  const char *nomeArquivo = "matrícula_selecaoRecursiva.txt";
  char content[100];
