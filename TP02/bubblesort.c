...
void bubble() {
    int i, j;
    for (i = (nArrJogadores-1); i > 0; i--) {
        for (j = 0; j < i; j++) {
            if (getAnoNascimento(arrJogadores[j]) > getAnoNascimento(arrJogadores[j+1])) {
                swap(j, j+1);
            }  else if (getAnoNascimento(arrJogadores[j]) == getAnoNascimento(arrJogadores[j+1]) && strcmp(getNome(arrJogadores[j]), getNome(arrJogadores[j+1])) > 0) {
                swap(j, j+1);
            }
        }
    }
...
void logs() {
  const char *nomeArquivo = "matrícula_bolha.txt";
  char content[100];
